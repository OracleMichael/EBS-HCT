...
- SSH key information† for each SSH key you need to log in to your Bastion hosts and VM servers:
    ...
    - (optional) passwords for the private keys
...
1. **Python version**: This program has been tested on Python 3.6.0-3.9.6 and requires Python 3.x to run. Performance on other versions may vary. If your machine does not have Python 3 installed, please install it - you can find the most up-to-date version for your OS at https://www.python.org/downloads/. To check if you have python, open a terminal window and run
    ```
    which python
    ```
    If the terminal gives you the empty string, you do not have python installed. To check the version of python, you may run:
    ```
    python
    ```
    then exit out of python by typing `exit()` or CTRL+D.
    ***IMPORTANT For some of you, "python" refers to python 2.7, and you may have python 3.x installed, which is aliased as "python3" (so you would run `which python3` and `python3` for checking the above). Going forward we will simply use "python" for all python commands, but please make sure to replace it with "python3" if your command is aliased as such.***
    To install Python, please follow these instructions:
    - Access the above link and download Python.
    ...
    python
    ```
    If the terminal does not immediately respond with the interactive Python interpreter (last line is `>>> `), please retry the installation above.
2. **Pip version**: This program uses pip to manage packages. Please ensure that it is installed and up-to-date. To install pip, you will need to download the get-pip.py script at https://bootstrap.pypa.io/get-pip.py, then run it as such in your terminal:
    ```
    python get-pip.py
    ```
    ...
    which pip
    ```
    You may have both python 2.x and 3.x installed; similarly "pip" will be the pip installer for python 2.x whereas "pip3" will be the alias for the pip installer for python 3.x. Therefore you would run `which pip3` and `pip3` for all pip commands. **As of September 3 2021, the link above gets you pip version 21.2.3, which is incompatible with python versions older than 3.6.**
...
**This section only applies to Windows users. For Mac/Linux users, please go to the above section**. If you are running these checks on your own machine, please ensure that the following prerequisites are satisfied. <!-- In the future, we intend to release this script on a VM to provision from the Oracle marketplace, so if you are choosing to use that to run this script, the only thing you will need to check is whether you are able to SSH into the VM from the marketplace image. -->

1. **Bash Terminal**: If you are using Windows, you will need to use a bash terminal. This is not the same as Windows Command Prompt or Windows PowerShell. You may need to download a separate terminal such as Git Bash: https://git-scm.com/downloads (the Windows downloadable is mainly for downloading Git, with Git Bash as a side option that you will need to check in the installation wizard). More detailed installation instructions for Git Bash below:
...
2. **Python version**: This program has been tested on Python 3.6.0-3.9.6 and requires Python 3.x to run. Performance on other versions may vary. If your machine does not have Python 3 installed, please install it - you can find the most up-to-date version for your OS at https://www.python.org/downloads/. To check if you have Python, open Git Bash and run
    ...
    ***IMPORTANT For some of you, "python" refers to Python 2.7, and you may have python 3.x installed, which is aliased as "python3" (so you would run `which python3` and `python3` for checking the above). Going forward we will simply use "python" for all python commands, but please make sure to replace it with "python3" if your command is aliased as such.***
    ...
    ```
    then close and reopen Git Bash so that the alias is loaded onto the new Git Bash window. You can test this by running the following in a Git Bash:
    ```
    python
    ```
    If the terminal does not immediately respond with the interactive Python interpreter (last line is `>>> `), please retry the installation above.
3. **Pip version**: This program uses pip to manage packages. Please ensure that it is installed and up-to-date. To install pip, you will need to download the get-pip.py script at https://bootstrap.pypa.io/get-pip.py, then run it as such in your terminal:
    ```
    python get-pip.py
    ```
    You can check if you have pip already by running:
...
1. Download this repository, then unzip the project once the download completes.
...
2. This program uses pip to manage packages. Please ensure that it is installed and up-to-date.
...
![Navigate To Check](images/hc4.png)
...
![Navigate To Check](images/hc5.png)
